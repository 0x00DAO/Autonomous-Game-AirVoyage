...
export class GameRoll extends GameObject {
    ...
    private set rollButtonStatus(value: GamePlayerStatus) {
        if (gameData.currentGame && gameData.currentGame.hasStarted) {
            if (gameData.currentGame.me) {
                this.txtLabel.string = GamePlayerStatus.Playing === value ? 'Roll' : 'Wait';
            } else {
                this.txtLabel.string = 'Join';
            }
        } else {
            this.txtLabel.string = GamePlayerStatus.Playing === value ? 'Roll' : 'Join';
        }
    }
    ...
    private async joinGame() {
        ...
        if (!player) {
            Toast.showMessage('Player status error.');
            return Promise.resolve();
        }
        ...
        try {
            if (GameData.INVALID_GAME_ID.eq(player.gameId)) {
                ...
                const toast = await Toast.showMessage('Waiting find or create game...', true);
